...
sys.stdout = open(os.path.join(OUT, "top_left_zone.txt"), "w", encoding="utf-8")
...
# 左上区全部文本 (x120-340, y80-270)
print("=== 左上区文本 (x120-340, y80-270) ===")
for t in texts:
    cx = (t["bbox"][0]+t["bbox"][2])/2; cy = (t["bbox"][1]+t["bbox"][3])/2
    if 120 <= cx <= 340 and 80 <= cy <= 270:
...
def dump_net_near(x, y, label=""):
    ...
    print("   labels: %s" % sorted(txs)[:50])
    for s in ss[:60]:
        print("    (%8.2f,%7.2f)->(%8.2f,%7.2f)" % (s["x1"], s["y1"], s["x2"], s["y2"]))

dump_net_near(196.0, 112.0, "B13-net")
dump_net_near(196.0, 241.5, "B14-net")
dump_net_near(213.0, 122.0, "DC1-top")
dump_net_near(213.0, 233.0, "DC1-bot")
