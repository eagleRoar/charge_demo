...
void Do_AdcSample(void)
{
	...
	{
		/* LINEAR_LI充电状态: 100μs足够, 防止电容充电至OPEN产生伪跳变
		   DETECT/IMP_CHECK/IMP_DIODE_TEST: 2000μs让电容充到可区分电池类型的电平 */
	...
	if(ADC_OK == test_adc)
	{
		/* LINEAR_LI CC/CV伪OPEN消噪: 保护IC触发→BxAD电容充电至VCC→伪OPEN.
		   读到OPEN时不更新, 保留上次有效值, 防止g_slotRefV被污染导致PEAK_DROP误判 */
